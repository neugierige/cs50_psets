...
char* indexes(const char* path)
{
    // printf("path in indexes: %s\n", path);

    char* html_path = calloc(strlen(path) + 11, 1);
    html_path = strcat(html_path, path);
    if (path[strlen(path) - 1] != '/') {
        html_path = strcat(html_path, "/");
    }
    html_path = strcat(html_path, "index.html");
    ...
    if (access(html_path, R_OK) == 0) {
        ...
        char* php_path = calloc(strlen(path) + 10, 1);
        php_path = strcat(php_path, path);
        if (path[strlen(path) - 1] != '/') {
            php_path = strcat(php_path, "/");
        }
        php_path = strcat(php_path, "index.php");
        return php_path;
    }
}
...
const char* lookup(const char* path)
{
    if (path != NULL)
    {
        char* extension = strrchr(path, 46);
